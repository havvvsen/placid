...
func RegisterUserHandler(server *placid.Server) {
	server.App.Post(server.Cfg.Endpoints.User, func(c fiber.Ctx) error {
		ctx := c.Context()
		...
		if c.HasBody() {
			...
			if user.Token.Valid {
				if user.Token.String == request.Token {
	...
	server.Logger.Info(fmt.Sprintf("Registered %s", server.Cfg.Endpoints.User))
}

func RegisterDeleteAccountHandler(server *placid.Server) {
	server.App.Delete(server.Cfg.Endpoints.DeleteAccount, func(c fiber.Ctx) error {
		ctx := c.Context()
	...
	server.Logger.Info(fmt.Sprintf("Registered %s", server.Cfg.Endpoints.DeleteAccount))
}
